...
const PRIVACY: LegalDocument = {
	...
	blocks: [
		...
		"**Copyright notices and counter-notices:** kept after the work they were about is deleted, and after either account is gone. The same reasoning as safety records, plus one specific to copyright: we are required to have a policy for ending the accounts of repeat infringers, and a record that the person accused can erase — by deleting the work — is not a record. We have not set an end date for these, and we would rather say so than name one we have not thought through.",
