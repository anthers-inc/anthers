...
const PRIVACY: LegalDocument = {
	...
	blocks: [
		...
		"**Checking uploaded images against known child sexual abuse material sends a fingerprint, never your file.** We calculate a **perceptual hash** of an image — a short mathematical fingerprint, not the picture — and send only that to the **Canadian Centre for Child Protection**, which tells us whether it matches known material. **Your files themselves never leave Anthers for this, and neither does anything that would let the Centre fetch them.** A fingerprint that does not match, which is nearly all of them, tells them nothing whatsoever about you or what you uploaded. **Video and audio are not covered**, because the fingerprint is an image hash: a video's thumbnail is checked and its frames are not.",
