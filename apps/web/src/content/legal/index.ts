...
const PRIVACY: LegalDocument = {
	...
	blocks: [
		...
		"## What we collect",
		"**Information you give us.** Your email address and the handle of the identity your account is built on — that handle is your address here, the name your profile hangs off. Optionally: a display name, bio, avatar, header image, website, and location. All optional fields are free text you choose, and anything you put in them is public.",
		...
		"**If you turn on Adult work, one fact about your card.** Adult work is reachable only from an account shown to belong to an adult, and the only way we check is to ask Stripe whether a card saved to your account is **credit**-funded, because a card issuer requires the holder of a credit account to be 18. Nothing is charged. **We keep that the check passed, when it passed, and which method did it — nothing else.** We never ask for your date of birth, and we store nothing about the card: not its brand, not its last four digits, not even the answer Stripe gave. The card stays saved with Stripe, as any card you pay with does.",
		...
		"**An identity you brought.** Every account is built on an ATProto identity, either one Anthers issues (below) or one you already hold. If you signed up with a Bluesky or other ATProto identity, we store your handle and identifier, and the tokens needed to act for that identity with the permissions you granted.",
		"**A handle we issued you.** If you asked Anthers for a handle rather than bringing one, there is a second account on our identity server carrying your email address, a password we generated and hold sealed, and whatever you have stored in that identity's repository. It is on a machine we operate, so it is ours to look after and ours to delete — but the **identifier** underneath it is published to a public directory that nobody, including us, can remove anything from.",
		...
		"**If we find child sexual abuse material, we report it to the National Center for Missing & Exploited Children**, as federal law requires of every service like ours. That report is a disclosure about a person rather than a fingerprint. It carries the material, when and how it was uploaded, and what we hold about the account that uploaded it: its registration details and any linked identity, the network information on its sign-in sessions if those still exist, and our payment processor's identifiers for it. The National Center may pass a report on to law enforcement. **We do not tell the account holder that a report has been made.**",
		...
		"**A legal hold suspends every period above for whatever it covers.** When the law requires us to keep something, no scheduled deletion reaches it until that obligation ends — not the ones on this page, and not a deletion you asked for. Whatever was due to go in the meantime goes when the hold lifts. A hold comes from one of three places:",
		"- **A report of child sexual abuse material.** Federal law requires us to preserve what a report to the National Center for Missing & Exploited Children concerns for **one year**, and making the report is what starts that year. The hold covers the material, the account that uploaded it, and any report somebody made about it.\n- **A preservation request from law enforcement**, which obliges us to keep what we already hold for **ninety days**, renewable once. It entitles nobody to see anything: disclosure still takes the legal process described under *Who we share it with*.\n- **A lawsuit that has begun or is expected**, for as long as it runs.",
		"**A hold stops deletion and does nothing else.** It is not a suspension, and an account under one works exactly as it did.",
		...
		"**A handle we issued you is emptied, and this is the one place we cannot promise everything is gone.** Everything stored in that identity is deleted, your email address comes off it, and the name is released. What remains is the identifier itself, which lives in a public directory that has no delete — not for us either — and which anyone may already have copied. So the honest sentence is that we remove what we hold and cannot remove what we never controlled. If you have taken a recovery key for that identity, it is still yours and deleting your Anthers account does not end your control of it.",
		...
		"**We don't ask your age, and we don't verify it to give you an account.** That is deliberate. Verifying age properly means collecting identity documents or dates of birth from everyone, and we are not willing to hold that about you in order to check a fact about a few. It means someone under 13 could sign up by not telling us — we would rather say that plainly than claim a protection we don't have. **Identity verification is never required to use Anthers.**",
		"**Adult work is the one place we check that someone is an adult**, and the check is the card funding type described under *What we collect* rather than an identity document or a date of birth. It shuts out some adults — anyone whose only card is debit or prepaid, and some cards issued outside the US that report no funding type — and we accept that cost rather than ask everybody for identification.",
...
const TERMS: LegalDocument = {
	...
	blocks: [
		...
		"**Child sexual abuse material is prohibited absolutely, and so is using Anthers to sexually exploit a child in any other way**, including grooming a child or trafficking one. When we learn of it, **we take it out of reach at once and report it to the National Center for Missing & Exploited Children**, as federal law requires. What the report concerns is preserved for a year, which overrides our ordinary deletion; the [Privacy Policy](/privacy) says what that covers. We do not tell the account holder that a report has been made.",
...
const CREATOR_TERMS: LegalDocument = {
	...
	blocks: [
		...
		"**Nothing that sexually exploits a child, ever.** The general terms say what happens when we learn of it, and it is the same for a creator as for anybody: it is taken out of reach at once, reported to the National Center for Missing & Exploited Children, and preserved for as long as federal law requires.",
