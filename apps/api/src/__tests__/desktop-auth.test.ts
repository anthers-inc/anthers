...
describe("Desktop auth", () => {
	...
	// ── Polling enrolment, for a client with no callback ─────────────────────
	//
	// `anthersp2p` has no `anthers://` scheme and is routinely run over SSH, where neither
	// a deep link nor a loopback redirect can reach it. It polls instead — and because
	// polling has no browser→app hop to cross, the one-time code that exists to cross it
	// is not involved at all.

	it("hands a polling client its token once the browser confirms", async () => {
		const verifier = hex();
		const challenge = await pkceChallenge(verifier);
		...
		const start = await req("/api/auth/desktop/start", {
			method: "POST",
			headers: { "Content-Type": "application/json" },
			body: JSON.stringify({ challenge, label: "anthersp2p on seedbox" }),
		});
		expect(start.status).toBe(201);

		// 202 rather than 404 while waiting: the request is fine, the human has not acted.
		// A 404 would send a polling client away from a flow that is merely pending.
		const pending = await req("/api/auth/desktop/poll", {
			method: "POST",
			headers: { "Content-Type": "application/json" },
		...
		const done = await req("/api/auth/desktop/poll", {
			method: "POST",
			headers: { "Content-Type": "application/json" },
			body: JSON.stringify({ verifier }),
		});
		...
		// A real, usable bearer session — not merely a 200.
		const me = await desktopReq("/api/auth/me", token);
		expect((await me.json()).user.username).toBe(userName);

		// And spent: the row is burned on handover, so a second poll gets nothing.
		const again = await req("/api/auth/desktop/poll", {
			method: "POST",
			headers: { "Content-Type": "application/json" },
			body: JSON.stringify({ verifier }),
		});
	...
	it("gives nothing to someone holding only the challenge", async () => {
		// 🚨 The reason the poll is keyed on the VERIFIER. The challenge travels in the
		// authorize URL — address bar, history, whatever the user forwarded it through — so
		// a challenge-keyed poll would let anyone who saw the link fetch the code and burn
		// the row with a wrong verifier, stranding the real client permanently.
		const verifier = hex();
		const challenge = await pkceChallenge(verifier);
		...
		await req("/api/auth/desktop/start", {
			method: "POST",
			headers: { "Content-Type": "application/json" },
			body: JSON.stringify({ challenge, label: "victim" }),
		});
		await req("/api/auth/desktop/authorize", {
			method: "POST",
			headers: { "Content-Type": "application/json", Origin: ORIGIN, Cookie: cookie },
			body: JSON.stringify({ challenge }),
		});

		// The challenge is a hash, so presenting it as a verifier hashes to something else.
		const attacker = await req("/api/auth/desktop/poll", {
			method: "POST",
			headers: { "Content-Type": "application/json" },
			body: JSON.stringify({ verifier: challenge }),
		});
		expect(attacker.status).toBe(404);

		// And the real client is untouched — nothing was consumed on its behalf.
		const real = await req("/api/auth/desktop/poll", {
			method: "POST",
			headers: { "Content-Type": "application/json" },
			body: JSON.stringify({ verifier }),
		});
		expect(real.status).toBe(200);
		expect((await real.json()).user.username).toBe(userName);
	});

	it("refuses to poll an enrolment that was never started", async () => {
		const res = await req("/api/auth/desktop/poll", {
			method: "POST",
			headers: { "Content-Type": "application/json" },
			body: JSON.stringify({ verifier: hex() }),
		});
		expect(res.status).toBe(404);
	});

	it("polls with no Origin, because the CSRF exemption is what makes it reachable", async () => {
		// The whole point of the exemption: this client cannot send an allowed Origin. If
		// the exemption were dropped, every poll would 403 and the flow would hang forever
		// on a screen that says it succeeded.
		const verifier = hex();
		const challenge = await pkceChallenge(verifier);
		await req("/api/auth/desktop/start", {
			method: "POST",
			headers: { "Content-Type": "application/json" },
			body: JSON.stringify({ challenge, label: "no-origin" }),
		});
		const res = await req("/api/auth/desktop/poll", {
			method: "POST",
			headers: { "Content-Type": "application/json" },
