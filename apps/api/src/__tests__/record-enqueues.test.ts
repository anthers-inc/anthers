...
describe("what each route asks for", () => {
	...
	it("asks for a follow's record, and carries its address into the removal on unfollow", async () => {
		sent = [];
		expect(
			(await call("POST", `/api/accounts/users/${await handleOf(hostName)}/follow`, abe.cookie))
				.status,
		).toBe(201);
		const [row] = await db
		...
		sent = [];
		expect(
			(await call("POST", `/api/accounts/users/${await handleOf(hostName)}/unfollow`, abe.cookie))
				.status,
		).toBe(204);
		expect(to(QUEUES.REMOVE_ATPROTO_RECORD)).toEqual([
	...
	it("takes down only the blocker's own follow record when they block somebody", async () => {
		...
		sent = [];
		expect(
			(await call("POST", `/api/accounts/users/${await handleOf(beeName)}/block`, abe.cookie))
				.status,
		).toBe(201);
		expect(to(QUEUES.REMOVE_ATPROTO_RECORD)).toEqual([
