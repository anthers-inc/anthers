...
try {
	...
	// Log which DB/user/server this job targets — cheap, and invaluable for confirming
	// the app's DATABASE_URL actually points at the intended cluster.
	const [info] =
		await client`SELECT current_database() AS db, current_user AS usr, inet_server_addr()::text AS server`;
	console.log(`migrate target: db=${info?.db} user=${info?.usr} server=${info?.server}`);
