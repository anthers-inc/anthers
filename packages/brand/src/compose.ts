...
//
// Low-level icon composition — turn a curated brand asset into standalone SVG
// markup, a data URI, or a `<g>` fragment for splicing into a larger SVG. Kept
// framework-agnostic (plain strings) so every surface can render it. `decor.ts`
// builds on `iconGroup` here; `index.ts` re-exports all of this as the package's
// public API.
