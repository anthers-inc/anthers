...
// Stamped with a LicenseRef rather than with the repository's Apache-2.0, because every
// geometry in it is Noun Project art that Anthers cannot relicense. ⚠️ Not `CC-BY-3.0`: the
// icons are offered on the Noun Project under CC BY 3.0, but Anthers holds them under the
// Noun Project's Royalty-Free License through its NounPro subscription, which is those terms
// without the attribution requirement (https://thenounproject.com/legal/). There is no SPDX
// identifier for that license, so it is named with a LicenseRef.
const generated = `// SPDX-License-Identifier: LicenseRef-NounProject-RoyaltyFree
