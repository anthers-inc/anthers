...
//
// ⭐ **This is what stops the license record being typed.** It was hand-maintained
...
// network.** `provenance.test.ts` runs its audit in CI, which has no key, and it has to be
// able to.
...
/** The source art for every curated icon, committed beside the markup generated from it. */
export const SVG_ROOT = join(BRAND_DIR, "svg");
...
export const PROVENANCE_NOTE =
	"Who drew each icon in packages/brand/svg, under what license, and where the original " +
	"lives. GENERATED by `bun run brand:add` from the Noun Project API — do not edit by hand. " +
	"scripts/noun/provenance.test.ts fails if an entry is missing or if a license is not one " +
	"Anthers may use the icon under.";
